...
- **In flight (being keyed):** fal `FAL_KEY` (code wired, no-op until keyed); Upstash `UPSTASH_REDIS_REST_URL`/`_TOKEN` (`probable-lemur-138225`, not yet wired); Sentry `SENTRY_DSN`/`NEXT_PUBLIC_SENTRY_DSN`/`_AUTH_TOKEN`/`_ORG`/`_PROJECT` (build-wrapped, runtime no-op); Inngest `INNGEST_EVENT_KEY`/`_SIGNING_KEY` (no account yet)
...
## IX.3 — Corroborations & corrections  ↪ confirms Z1/VII.6, refines §6/A8
