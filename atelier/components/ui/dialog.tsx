...
>(({ className, style, ...props }, ref) => {
  const mergedStyle: React.CSSProperties = {
    backgroundColor: 'hsl(var(--vibe-ink, 0 0% 0%) / 0.62)',
    ...
    <DialogPortal>
      ...
      >
        ...
        <DialogPrimitive.Close
          className="absolute right-4 top-4 opacity-70 transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:pointer-events-none"
          style={{
            color: 'hsl(var(--vibe-ink, var(--foreground)))',
            borderRadius: 'var(--vibe-radius-button, 6px)',
          }}
        >
          <X className="h-4 w-4" />
...
>(({ className, style, ...props }, ref) => {
  const mergedStyle: React.CSSProperties = { ...headingStyle(), ...(style ?? {}) };
  return (
