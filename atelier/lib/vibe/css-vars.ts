...
export function vibeCss(vibe: Vibe | null | undefined): VibeCssVars {
  ...
  // font_pairing wins over typography when both are present (explicit choice).
  ...
  // Mono is always emitted so joke/gag cards can opt-in regardless of body dial.
...
/** Motion-scale as a number for framer-motion. Mirrors `--vibe-motion-scale`. */
