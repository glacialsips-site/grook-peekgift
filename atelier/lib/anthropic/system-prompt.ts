 ...
 *
 *   - Block 1: BASE_PROMPT — canonical, very stable.
 ...
 *     state, phase, anon turns remaining, curator memory).
 *
 * Cache control: a single `cache_control: ephemeral` marker is preserved
 * on Block 1 (matching the existing prompt-cache hook). Packet 40 owns
 * the full caching strategy — this builder does NOT add additional
 * markers on Blocks 2 / 3 / 4.
 *
...
export function buildSystemBlocks(
  ...
  // Block 1 — base prompt (cache marker preserved; packet 40 owns the full
  // caching strategy)
  blocks.push({
    type: 'text',
    text: BASE_PROMPT,
  ...
  if (conditionalText) {
    blocks.push({ type: 'text', text: conditionalText });
  }

  // Block 4 — per-turn dynamic context
