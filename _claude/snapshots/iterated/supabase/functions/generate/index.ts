...
function refineFraming(currentHtml: string, instruction: string) {
  return (
...
Deno.serve(async (req: Request) => {
  ...
  if (!key) return new Response("ANTHROPIC_API_KEY not set", { status: 500, headers: CORS });

  const body = await req.json().catch(() => null);
  if (!body || !Array.isArray(body.messages)) return new Response("bad request", { status: 400, headers: CORS });

  let apiMessages: Array<{ role: string; content: unknown }>;
  if (body.currentHtml && typeof body.currentHtml === "string") {
    const lastUser = [...body.messages].reverse().find((m: any) => m && m.role === "user");
  ...
  if (!apiMessages.length) return new Response("no messages", { status: 400, headers: CORS });
  ...
  const stream = new ReadableStream({
    async start(controller) {
      const send = (s: string) => { try { controller.enqueue(enc.encode(s)); } catch (_) {} };
      send(": warming up\n\n");
